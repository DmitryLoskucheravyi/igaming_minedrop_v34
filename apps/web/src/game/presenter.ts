...
/* Експортуємо: ХУД малює по стрілці на КОЖЕН режим понад звичайний, і
   рахувати їх він мусить із цього ж списку. Інакше додали швидкість —
   а індикатор мовчки лишився на стільки стрілок, скільки було. */
export const SPEEDS = [1, 2, 3, 4, 10, 25];
...
/* ---- розмір кірки на полі ----
   ...
   Історія: 1.5 полотна старого скіну (малюнок займав у ньому 0.8125,
   тобто видимих 1.22 клітинки) -> 1.22 на нових скінах, намальованих
   упритул до країв -> 0.81, кірку зменшено в півтора раза -> 0.93,
   назад на 15% (0.81 * 1.15): на 0.81 вона загубилась серед блоків.

   ЦЕ ЧИСЛО ХОДИТЬ У ПАРІ З bodyR (packages/engine/config.ts): видимий
   розмір задає воно, радіус зіткнення — bodyR, і вони мусять
   описувати той самий предмет. Розійшлись — і гра або ламає блок, до
   якого кірка ще не долетіла, або не помічає дотику, який гравець уже
   бачить. Тому зменшення спрайту в 1.5 раза тягне за собою таке саме
   зменшення bodyR (0.55 -> 0.37), а це вже СЕРВЕРНА величина: вона
   переписує траєкторії, віддачу й усі збережені реплеї, тож після
   такої зміни обов'язковий новий прогін sim/final.ts. Так само й
   зворотний крок: 0.81 -> 0.93 тягне bodyR 0.37 -> 0.425. */
const PICK_VIS = 0.93;
...
/* 3.2 -> 5.5: спалах іде за розміром самого вибуху, а той побільшав
   (TNT_POWER_* у run.ts). Менший спалах над вирвою на 8-13 клітинок
   читався б як осічка. */
const FX_TNT_SIZE = 5.5;
const FX_TNT_LIFE = 0.3;
/* Слід — силует самої кірки, тому їде за її розміром: 2.0 -> 1.33 і
   0.45 -> 0.30 разом зі зменшенням PICK_VIS у півтора раза, а тепер
   1.53 і 0.345 разом із поверненням 15% (× 1.15). Лишити їх як були
   означало б шлейф не того розміру, що сама кірка. */
const FX_TRAIL_SIZE = 1.53;
