...
import { BLOCKS, CONFIG, TIERS, type Tier } from '@minedrop/engine';
...
class AssetStore {
  ...
  private corePaths(): Record<string, string> {
    ...
    /* Блоки-множники — по картинці на кожен ікс із таблиці. У BLOCKS
       вони одним записом `mult` без skin, бо в сітці це справді ОДИН
       блок: ікс лежить у самій клітинці (cell.m), а не в її типі.

       Список беремо з CONFIG.mult.table, а не перелічуємо руками:
       додали в таблицю новий множник — тут він з'явиться сам, і
       лишиться тільки покласти файл. Якщо файлу немає, Render.multBlock
       намалює блок кодом, як раніше.

       У корі, а не в extras: множники трапляються з перших рядів, і
       підвантаження на льоту було б видно як мигання. */
    for (const { m } of CONFIG.mult.table) p['block.mult.' + m] = '/blocks/mult-' + m + '.png';
