...
/* ============================================================
   ГЕОМЕТРІЯ КІЛЬЦЯ (public/ui/reel-ring.png, 557x561)
   ...
   Непрозорий bbox: x 2..552, y 8..555. Центр отвору (заливка від
   середини по прозорих пікселях): 277.0, 281.5.
   ...
   Радіус вирізу: медіана «першого непрозорого пікселя» по 720
   променях = 185 px, розкид 183..191 (виріз намальований від руки й
   не є ідеальним колом). Беремо 186 = 0.334 ширини: трохи більше за
   медіану, бо кільце й так накриває край.

   Ім'я файлу («перевернути на 90 град») стосувалось вихідного
   експорту — у тому вигляді, в якому файл лежить у репозиторії,
   зелені стрілки лінії виплати вже стоять зверху й знизу вирізу.
   Повертати НЕ треба, і в коді цього повороту немає.
   ============================================================ */
const FRAME_W = 557, FRAME_H = 561;
export const FRAME_ASPECT = FRAME_W / FRAME_H;

/** центр вирізу — частка від розмірів кільця */
export const FRAME_WIN_CX = 0.4973;   // 277.0 / 557
export const FRAME_WIN_CY = 0.5018;   // 281.5 / 561
/** радіус вирізу — частка ШИРИНИ кільця */
export const FRAME_WIN_R = 0.334;     // 186 / 557
...
/* ============================================================
   ...
   Раніше це була текстова пігулка «3/7 до гарантии» під полем, потім
   рубіни на вінку. Тепер те саме показують СЕГМЕНТИ САМОГО КІЛЬЦЯ: за
   кожну пусту ставку в чергову заглиблену панель лягає червона
   паличка, а коли набралось на гарантію — усі сім стають зеленими.

   Сегментів на кільці рівно СІМ, і це не збіг: CONFIG.pity теж 7.
   Один сегмент = одна пуста ставка.

   ---- де саме лежить панель ----
   Радіальний профіль яскравості кільця (усереднено по центрах усіх
   семи сегментів) читається так:
     r 182..198  тонкий темний обідок навколо вирізу;
     r 199..213  СВІТЛИЙ внутрішній рант (той самий колір, що й
                 перемички між сегментами);
     r 215..249  ТЕМНА ЗАГЛИБЛЕНА ПАНЕЛЬ — сюди й лягає паличка;
     r 250..282  світлий зовнішній рант.
   Перемичка між сегментами — світла поперечина, що з'єднує
   внутрішній рант із зовнішнім; саме тому вона й обмежує панель.
...
/* Кути перемичок, градуси, canvas-конвенція (0° = праворуч, кут росте
   ...
   світлих плям на радіусах 222..248:
     0.05, 51.50, 98.80, 151.90, 207.65, 261.15, 308.60
   Кроки між ними 47.3°..55.75° — кільце намальоване від руки, тож
   рівномірних 360/7 тут НЕМАЄ і підставляти їх не можна: перекіс
   видно неозброєним оком. */

/* Сегменти в ПОРЯДКУ ЗАПОВНЕННЯ: за годинниковою, починаючи з
   верхнього (у canvas-координатах 270° = вгору, найближчий до нього
   центр — 284.88°). deg — центр сегмента, span — його кутова ширина
   (відстань між сусідніми перемичками). */
interface Seg { deg: number; span: number }
const SEGMENTS: readonly Seg[] = [
  { deg: 284.88, span: 47.45 },   // верх
  { deg: 334.33, span: 51.45 },
  { deg: 25.77, span: 51.45 },
  { deg: 75.15, span: 47.30 },
  { deg: 125.35, span: 53.10 },
  { deg: 179.78, span: 55.75 },
  { deg: 234.40, span: 53.50 },
];

/* Куди й наскільки великою лягає паличка. Числа підібрані накладанням
   палички на кільце й переглядом результату, від початкових значень
   із заміряної панелі (центр 232, товщина 34):

     PIP_R  226/557 — центр прямокутника палички по радіусу. Менший за
            середину панелі (232) навмисно: паличка намальована
            ДУГОЮ, і її bbox по радіусу — це товщина ПЛЮС стрілка
            прогину, тобто центр bbox лежить ближче до центру кільця,
            ніж середина самої дуги.
     PIP_W  51/557 — ширина прямокутника (радіальний бік). Це знову ж
            таки товщина + прогин, а не сама товщина.
     PIP_GAP_DEG — скільки градусів сегмента лишити перемичці й
            просвіту біля неї (перемичка ~4.3° завширшки).

   Не сходиться після заміни арту — крутити треба саме ці три числа;
   кути сегментів чіпати не можна, вони заміряні. */
const PIP_R = 226 / FRAME_W;
const PIP_W = 51 / FRAME_W;
const PIP_GAP_DEG = 3.5;

/* Непрозорий bbox самих паличок — беремо з файлу тільки його. Прозорі
   поля (а їх у цих файлах багато й вони несиметричні) з'їли б усю
   підгонку: прямокутник розтягувався б по полях, а не по малюнку. */
const PIP_ART = {
  green: { key: 'pipGreen', sx: 8, sy: 63, sw: 217, sh: 485 },    // файл 239x561
  red: { key: 'pipRed', sx: 17, sy: 20, sw: 226, sh: 500 },       // файл 260x560
} as const;

const PIP_GLOW = { green: '125,255,154', red: '255,90,74' };
...
/* ---- ПІДКЛАДКА ПІД СТРІЧКУ (ui/reel-backing.png) ----
   ...
   треба саме за цим радіусом, а не за габаритами картинки: інакше
   між краєм підкладки й кільцем лишається щілина.
...
export class Reel {
  ...
  private segsFor(need: number): readonly Seg[] {
  ...
  drawPips(
    ...
    const R = frameW * PIP_R;
    const pw = frameW * PIP_W;
    ...
    for (let i = 0; i < segs.length; i++) {
      ...
      const on = done || i < got;
      const arc = ((seg.span - PIP_GAP_DEG) * Math.PI) / 180;
      const ph = arc * R;                       // довжина дуги сегмента

      if (!on) {
        // порожній сегмент трохи притемнюємо — інакше «є» й «немає» не
        // відрізнити: сама панель темна й без палички
        ctx.save();
        ctx.strokeStyle = 'rgba(0,0,0,.25)';
        ctx.lineWidth = pw;
        ctx.beginPath();
        ctx.arc(winCx, winCy, R, a - arc / 2, a + arc / 2);
        ctx.stroke();
        ctx.restore();
        continue;
      }

      const art = i < greenN ? PIP_ART.green : PIP_ART.red;
      const img = Assets.get(art.key);
      ...
      ctx.save();
      ctx.translate(winCx + Math.cos(a) * R, winCy + Math.sin(a) * R);
      /* Той самий кут, що й у сегмента: картинка намальована
         ВЕРТИКАЛЬНО й вигнуто, і після повороту її вигин лягає точно
         вздовж дуги — перевірено накладанням на кільце. Дзеркалити
         нічого не треба, одна картинка обслуговує всі сім позицій. */
      ctx.rotate(a);
      const w = pw * k, h = ph * k;
      if (img) {
        ctx.drawImage(img, art.sx, art.sy, art.sw, art.sh, -w / 2, -h / 2, w, h);
      } else {
        // картинки немає — лишається кольорова смуга, щоб прогрес читався
        ctx.fillStyle = i < greenN ? '#3ddc6e' : '#e04a3c';
        ctx.fillRect(-w / 2, -h / 2, w, h);
      }

      /* Світіння спалаху — ДОДАВАННЯМ поверх палички, а не її
         прозорістю: паличка мусить лишатись непрозорою, інакше в
         «темній» фазі з-під неї проступала б панель. */
      if (flash > 0) {
        ctx.globalCompositeOperation = 'lighter';
        const glow = PIP_GLOW[i < greenN ? 'green' : 'red'];
        const g = ctx.createLinearGradient(-w / 2, 0, w / 2, 0);
        g.addColorStop(0, `rgba(${glow},0)`);
        g.addColorStop(0.5, `rgba(${glow},${(0.5 * flash).toFixed(3)})`);
