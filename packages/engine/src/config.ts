...
export const CONFIG = {
  ...
  buy: {
    ...
    /* ЦІНИ ВИЩІ ЗА «ЧЕСНІ» — це продуктове рішення, не помилка.

       Чесні ціни (віддача бонуски = звичайній грі) за sim/buy.ts:
       1.7 / 4.4 / 8 / 12.5 / 13.2. Поточні підняті приблизно у 8-9
       разів: бонуска зроблена преміальною покупкою, а не дешевшим
       способом отримати кірку.

       ВАЖЛИВО, ЩО З ЦЬОГО ВИПЛИВАЄ: віддача бонуски тепер помітно
       нижча за звичайну гру. Перевіряти після кожної зміни цін —
       npm run sim:buy.

       Ще одне: алмазна кірка заробляє лише трохи більше за золоту
       (x10.14 проти x9.58), тому чим сильніше задирати саме її ціну,
       тим гіршою покупкою вона стає. Щоб верхній тір був вигідним, а
       не просто дорогим, треба піднімати не ціну, а його ЗАРОБІТОК. */
    price: {
      lvl2: 13,      // середня виплата x1.34
      lvl3: 30,      // x3.38
      lvl4: 55,      // x6.19
      gold: 90,      // x9.58
      diamond: 120,  // x10.14
