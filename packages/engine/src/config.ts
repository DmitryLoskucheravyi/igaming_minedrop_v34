...
export const CONFIG = {
  ...
  /* Дільник виплати — головна ручка RTP. Зв'язок оберненo лінійний
     ...
     назад із базової гри: 88% тепер складаються приблизно як 72.5%
     звичайних прокрутів + 17.7% бонуски (sim/scatter-sweep.ts).
  ...
  buy: {
    ...
    multBoost: 4,        // частота блоків-множників: 1.6 -> 4 (у 2.5 раза більше)
    enchantBoost: 2.6,   // частота столів зачарування: 1.6 -> 2.6
    ...
    tailBoost: { 10: 2.5, 15: 2, 25: 1.8, 50: 1.8 } as Record<number, number>,
    /* Ціни зведені так, щоб віддача бонуски дорівнювала звичайній грі
       (~78%): купівля має бути іншим ТЕМПОМ, а не вигіднішою чи
       гіршою ставкою. Заміряно sim/buy.ts на 20k забігів.

       Віддача при цих цінах: 75 / 79 / 78 / 79 / 75 відсотків.

       Алмазна коштує трохи БІЛЬШЕ за чесну (120.1) навмисно: вона
       заробляє практично стільки ж, скільки золота (x93.8 проти
       x94.7 — різниця в межах шуму), і якби ціна йшла строго за
       заміром, верхній тір виявився б дешевшим за попередній. Сходинка
       вниз у прайсі читається як помилка, тому лишаємо 125.

       ПЕРЕРАХОВУВАТИ після будь-якої зміни фізики, ваг блоків або
       щедрості бонусної шахти — саме вони й рухають ці числа. */
    price: {
      lvl2: 11,      // середня виплата x8.26
      lvl3: 35,      // x27.54
      lvl4: 76,      // x59.27
      gold: 120,     // x94.72
      diamond: 125,  // x93.76
