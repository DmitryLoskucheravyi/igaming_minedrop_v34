...
/* ---- ЗРИВ ІЗ ВЕРТИКАЛІ ----
   ...
   складової, і після STRAIGHT_SEC зриваємо її з вертикалі. Напрямок —
   ...
   STRAIGHT_SEC = 0.7 з сильним STRAIGHT_PUSH = 3.0.
...
export const STRAIGHT_SEC = 1.6;    // скільки терпіти вертикальний рух
export const STRAIGHT_VX = 1.3;     // нижче цього |vx| рух вважається вертикальним
export const STRAIGHT_PUSH = 1.4;   // база бічного імпульсу (затравка)
...
export class Run {
    ...
    if (Math.abs(p.vx) < STRAIGHT_VX) {
      ...
      if (p.straightT >= STRAIGHT_SEC) {
        ...
        /* Крутимо сильніше, ніж штовхаємо: далі кірку понесе вбік уже
           ...
           кувирка, а не ривком нізвідки. */
        p.rotV = clamp(p.rotV + dir * P.spinKick * 1.4, -P.maxSpin, P.maxSpin);
      }
    } else {
      p.straightT = 0;
    }
  ...
  private collide(p: Pick): void {
    ...
    if (!p.dead) {
      ...
      if (len > 1e-6 && (this.hitStrength > 0 || this.rubberKick > 0)) {
        const nx = sumX / len, ny = sumY / len;
        const k = this.hitBounce(p, nx, ny);
        if (this.rubberKick > 0) {
          /* Гума: нормальну швидкість ЗАДАЄМО (тому rest = 0), стелі
             підйому немає — це трамплін, а не відскок від блока. */
          this.resolveContact(p, R, nx, ny, 0, this.rubberKick * k, Infinity,
                              P.maxSideSpeed * CONFIG.rubber.speedUp);
        } else {
          const s = k * this.hitStrength;
  ...
  private impact(p: Pick, r: number, c: number, cell: Cell): void {
    ...
    this.hitStrength = Math.max(this.hitStrength, broke ? 1 : P.crackBounce);
    this.checkDead(p, idx);
  }
