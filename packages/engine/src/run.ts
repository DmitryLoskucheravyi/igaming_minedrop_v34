...
export class Pick {
  ...
  rubberT = 0;
  /* Скільки ще секунд кірка стоїть після удару в цінний блок. Ставить
     impact(), витрачає stepPick. Швидкість при цьому НЕ чіпається —
     після паузи рух продовжується точно таким самим. */
  freezeT = 0;
...
export class Run {
  ...
  private stepPick(p: Pick, dt: number): void {
    /* МІКРО-ФРІЗ. Кірка щойно вдарила в руду — на долю секунди вона
       просто стоїть: ні гравітації, ні руху, ні нових дотиків. Вихід
       саме тут, до всього іншого, бо зупинка має бути повною; інакше
       вийшло б не «завмерла», а «сповільнилась», а це вже інший ефект
       (див. oreFreeze у config).

       Швидкість не чіпаємо навмисно: після паузи кірка продовжує рух
       точно таким самим, тому траєкторія лишається тією ж, просто
       зсунутою в часі. */
    if (p.freezeT > 0) {
    ...
    /* Поле стінок. Діє ЗАВЖДИ, поки кірка в крайній смузі, а не лише
       на дотику — саме тому воно й лікує зависання: поштовх при ударі
       треба спершу заробити, а поле відганяє й тоді, коли кірка просто
       сповзає до краю. Сила лінійно спадає до нуля на межі смуги. */
    {
      const R = P.bodyR * p.scale;
      const left = p.x - R;
      const right = (this.mine.cols - R) - p.x;
      /* Сила залежить не лише від відстані, а й від того, наскільки
         кірка ЗАСТРЯГЛА. Поле, яке тисне на всіх однаково, просто не
         пускає її в крайні колонки — а там лежить така сама руда, як
         усюди, і забирати її з гри ми не хочемо. Тому множник гасне зі
         швидкістю: кірка, що летить уздовж краю, проходить вільно, а та,
         що ледве повзе біля стінки, отримує повний поштовх. */
      const stall = 1 - Math.min(1, Math.abs(p.vx) / P.maxSideSpeed);
      if (stall > 0) {
        if (left < P.wallPushRange) {
          p.vx += P.wallPush * stall * (1 - Math.max(0, left) / P.wallPushRange) * dt;
        }
        if (right < P.wallPushRange) {
          p.vx -= P.wallPush * stall * (1 - Math.max(0, right) / P.wallPushRange) * dt;
        }
      }
      /* Стелі тут НЕМАЄ навмисно. Обрізати vx до maxSideSpeed було б
         природно на вигляд, але це мовчки різало б відкид від вибуху й
         слайма: їм дозволено перевищувати звичайну стелю (tntSide,
         rubber.sideBoost), і якраз вони найчастіше й виносять кірку
         з-під стінки. Перевірено заміром: із клампом суцільне
         перебування біля краю зросло з 0.47 до 0.58 с. Само поле
         розігнати кірку не може — воно згасає до нуля на межі смуги. */
    }
    ...
    /* p.freezeT у стоп-умові: фріз ставить impact() усередині collide(),
       і без цього кірка доїхала б решту підкроків цього ж кадру вже
       «замороженою». */
    for (let i = 0; i < n && !p.dead && p.freezeT <= 0; i++) {
      p.x += p.vx * sdt;
      p.y += p.vy * sdt;
      this.collide(p);
    }
  ...
  private impact(p: Pick, r: number, c: number, cell: Cell): void {
    ...
    /* НАСКІЛЬКИ ПРУЖНИМ ВИЙДЕ ЦЕЙ ДОТИК — три рівні, і всі три про
       блок ПІД кіркою.

       Саме «під» тут суттєве. Глушити будь-який удар означало б гасити
       й ті, де кірка влетіла в стінку жили ЗБОКУ: там відскок якраз
       потрібен, інакше вона зависає в повітрі біля вертикальної грані
       й довбає її, ніби приклеєна. Вниз її тримає власна вага — і це
       виглядає як врубання; вбік не тримає ніщо — і це має виглядати
       як удар. (dy < 0 означає, що центр клітинки нижчий за кірку —
       вісь y дивиться вниз; !sideways — що дотик прийшовся переважно
       по вертикалі.)

         oreBounce   — блок ПРИНОСИТЬ ГРОШІ (value > 0). Кірка в нього
                       врубається, а не стрибає від нього. Діє й коли
                       жила тріснула, і коли розсипалась: раніше тут був
                       розрив — поки руда терпіла, відскок глушився, а
                       на останньому, найпомітнішому ударі ставав
                       повним, і кірку підкидало рівно в момент здобичі.
         crackBounce — блок вистояв, але грошей не дає (камінь, дерн):
                       прогризаємо, відскок слабкий.
         1           — усе інше: звичайний удар.

       Слайм у цей відбір не потрапляє за побудовою (value 0), та й
       його трамплін рахується окремою гілкою в collide(), повз
       hitStrength. */
    const under = !sideways && dy < 0;
    /* Удар у цінний блок під собою — мікро-фріз. Разом із oreBounce це
       й читається як «врубилась»: не відскочила й не загрузла, а
       зупинилась на мить і пішла далі. */
    if (under && def.value > 0) p.freezeT = P.oreFreeze;
    const strength = under && def.value > 0 ? P.oreBounce
      : !broke && under ? P.crackBounce
      : 1;
    /* max, а не min: hitStrength збирається по ВСІХ клітинках, яких
       торкнулись за крок. Якщо поруч із жилою зачепило камінь чи
       вертикальну грань — відскок має бути звичайним, глушити його
       через сусідню руду не можна. */
    this.hitStrength = Math.max(this.hitStrength, strength);
    this.checkDead(p, idx);
  }
