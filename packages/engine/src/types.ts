...
export interface RoundResult {
  ...
  bonusWon?: boolean;
  /* Раунд зіграно подарованим прокрутом із колеса щоденного бонусу:
     cost 0, ставка фіксована сервером, шахта звичайна. Не плутати з
     free — то безкоштовна БОНУСКА за скаттери. */
  gift?: boolean;
  /* У скільки разів була піднята ймовірність кірки (куплені фріспіни).
     Клієнт зобов'язаний передати це число в buildSetup, інакше він
     збере ІНШУ рулетку з того самого сида й розійдеться з сервером. */
  chanceX?: number;
  /* Раунд зіграно КУПЛЕНИМ прокрутом: cost 0 (заплачено за пакет
     наперед), ставка з пакета, шанс кірки подвоєний, а виграш лягає
     бонусними грошима — із замком до відіграшу. */
  paidSpin?: boolean;
