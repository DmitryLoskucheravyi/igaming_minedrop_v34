...
export interface Payment {
  ...
  adminNote?: string;
  /* ---- промокод ----
     Обидва поля ЗАМОРОЖЕНІ в момент створення заявки. Адмін може
     вимкнути код чи змінити відсоток, поки переказ іде мережею, — на
     вже видану обіцянку це не впливає: гравцеві показали «+20%»,
     значить, він отримає 20%. */
  /** код, як його записано в довіднику (у верхньому регістрі) */
  promo?: string;
  /** скільки відсотків від суми лягло бонусом */
  promoPercent?: number;
}
...
export interface WithdrawInfo {
  ...
  minRub: number;
  maxRub: number;
  /* Бонус, замкнений до відіграшу: скільки ₽ лежить під замком і як
     далеко до його зняття. Баланс при цьому один — розділення живе
     лише тут, у вікні виводу. */
  bonus: {
    locked: number; done: number; need: number;
    /** до якого часу треба відіграти (мс epoch); 0 — бонусу немає */
    until: number;
